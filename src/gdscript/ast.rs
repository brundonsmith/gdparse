...
impl Slicable for ExpressionOrIdentifier {
    fn slice(&self) -> &Slice {
        match self {
            ExpressionOrIdentifier::Expression(e) => e.slice(),
            ExpressionOrIdentifier::Identifier(i) => i.slice(),
        }
    }
}
...
impl Parentable for MatchCase {
    fn set_parent<TParentKind>(&mut self, parent: &AST<TParentKind>)
    where
        TParentKind: Clone + TryFrom<Any>,
        Any: From<TParentKind>,
    {
        self.pattern.set_parent(parent);
        self.outcome.set_parent(parent);
    }
}

impl Parentable for ExpressionOrIdentifier {
    fn set_parent<TParentKind>(&mut self, parent: &AST<TParentKind>)
    where
        TParentKind: Clone + TryFrom<Any>,
        Any: From<TParentKind>,
    {
        match self {
            ExpressionOrIdentifier::Expression(expr) => expr.set_parent(parent),
            ExpressionOrIdentifier::Identifier(ident) => ident.set_parent(parent),
        }
    }
}
...
pub struct EnumDeclaration {
    ...
    pub members: Vec<(AST<PlainIdentifier>, Option<AST<Expression>>)>,
}
...
pub struct MatchCase {
    pub pattern: AST<MatchPattern>,
    pub outcome: AST<Block>,
}

#[derive(Debug, Clone, PartialEq)]
pub enum MatchPattern {
    Single(ExpressionOrIdentifier),
    Multi(Vec<ExpressionOrIdentifier>),
    Array {
        members: Vec<ExpressionOrIdentifier>,
        ellipsis: bool,
    },
    Dict {
        entries: Vec<(AST<Expression>, ExpressionOrIdentifier)>,
        ellipsis: bool,
    },
}
